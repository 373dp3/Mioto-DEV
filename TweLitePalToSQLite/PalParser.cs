...
public class PalParser : PacketCommon, IMonoPacket, IComResultReceiver
{
    TweLitePalToSQLite.DbWrapper p = TweLitePalToSQLite.DbWrapper.getInstance();
    ...
    public void receiveResult(string line)
    {
        int ofs = 1;
        this.clear();
        this.parse(line, ref ofs);
        d(this.ToString());
        if (this.lqi == 0) { return; }

        p.insert(new TweLitePalToSQLite.TableTweLitePal()
        {
            mac = this.mac,
            battery = this.batt,
